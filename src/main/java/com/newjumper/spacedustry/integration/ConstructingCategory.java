...
public class ConstructingCategory implements IRecipeCategory<ConstructingRecipe> {
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, ConstructingRecipe recipe, IFocusGroup focuses) {
        ...
        builder.addSlot(RecipeIngredientRole.OUTPUT, 68, 19).addItemStack(RecipeUtil.getResultItem(recipe));
    }

    @Override
    public void draw(ConstructingRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
        this.progress.getUnchecked((time - 10) / 3).draw(guiGraphics, 24, 14);
